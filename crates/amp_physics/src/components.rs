//! Vehicle physics components for the Amp game engine.
//!
//! This module provides all the necessary components for implementing
//! realistic vehicle physics including engines, transmissions, wheels,
...
/// Marker component for wheel entities.
///
/// This component identifies an entity as a wheel and enables
/// wheel-specific physics systems to process it.
#[derive(Component, Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct Wheel;
...
// SuspensionRay component moved to suspension.rs to avoid duplicate exports
...
mod tests {
    ...
    #[test]
    fn vehicle_component_creation() {
        let vehicle = Vehicle;
        assert_eq!(vehicle, Vehicle);
    }

    #[test]
    fn wheel_component_creation() {
        let wheel = Wheel;
        assert_eq!(wheel, Wheel);
    }

    // SuspensionRay test moved to suspension.rs
    ...
    #[test]
    fn marker_components_serialization() {
        let vehicle = Vehicle;
        let wheel = Wheel;

        // Test that marker components can be serialized/deserialized
        let vehicle_json = serde_json::to_string(&vehicle).unwrap();
        let wheel_json = serde_json::to_string(&wheel).unwrap();

        let vehicle_deserialized: Vehicle = serde_json::from_str(&vehicle_json).unwrap();
        let wheel_deserialized: Wheel = serde_json::from_str(&wheel_json).unwrap();

        assert_eq!(vehicle, vehicle_deserialized);
        assert_eq!(wheel, wheel_deserialized);
    }
