...
pub fn setup_audio_systems(
    ...
    let audio_channels = AudioChannels::new(&audio);
...
pub fn update_engine_audio(
    ...
    for (mut engine_audio, vehicle_engine, transform) in engine_query.iter_mut() {
        ...
        if vehicle_engine.throttle > 0.0 && !engine_audio.is_playing {
            if let Some(sound_handle) = audio_assets.engine_sounds.get("default") {
                audio
                    .play_in_channel(sound_handle.clone(), &audio_channels.engine)
                    .looped()
                    .with_volume(target_volume)
                    .with_playback_rate(engine_audio.pitch_multiplier);
                engine_audio.is_playing = true;
            }
        } else if engine_audio.is_playing {
            // Update existing sound parameters
            audio
                .set_volume_in_channel(target_volume, &audio_channels.engine)
                .set_playback_rate_in_channel(engine_audio.pitch_multiplier, &audio_channels.engine);
        }

        // Stop engine sound if throttle is 0
        if vehicle_engine.throttle == 0.0 && engine_audio.is_playing {
            audio.stop_channel(&audio_channels.engine);
            engine_audio.is_playing = false;
        }
    }
}
...
pub fn update_music_system(
    ...
    for mut music_system in music_query.iter_mut() {
        // Handle music playback logic
        if music_system.is_playing && !music_system.playlist.is_empty() {
            // Music system logic would go here
            // For now, just maintain the component state
        }
    }
}
